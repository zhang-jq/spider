...
public class IchemistryPipeline implements Pipeline {
    ...
    private static final String SITE_TYEP = "ichemistry";
...
//
//        }
        List<String> productUrls = resultItems.get("productUrls");
        if (productUrls != null) {
            productUrls.stream()
                    .map(url -> ProductUrl.builder().url(url).type(SITE_TYEP).build())
                    .forEach(productUrl -> productService.saveProductUrl(productUrl));
        }
